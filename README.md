...
a white canvas. A fixed harness rasterizes and scores every candidate (RMSE, which
the model cannot fake), the model proposes full replacement programs
...
![The champion at 80 vs 1000 triangles vs target](docs/budget_1000.png)

*The champion generalises across budgets it never saw during evolution: the
heuristic reads `t` and `n_total`, so its coarse-to-fine strategy rescales.
With the budget raised from 80 to 1000 triangles it places 987 of them and
takes the hummingbird from RMSE 0.114 to 0.076 in about 3 seconds.*

![Per-triangle build-up of the 1000-triangle hummingbird](docs/hero_1000.gif)

*The same 1000-triangle run, one frame per placement early and sparser later:
big tonal blocks, then the silhouette, then feathers.*
...
run: 0.240 → 0.139, beating a reference heuristic (0.162) that
was written only to validate headroom and never shown to the model.*
